...
- Phase-0 Komponenten sind teilweise implementiert: Das Prozess-Shim spannt jetzt Overlay-Workspaces und eine konfigurierbare Seccomp-BPF-Filterung auf (`crates/cave-kernel/src/lib.rs:604-1216`). Neu hinzugekommen sind Lifecycle-Tests, die den Prozess-Runtime ohne Namespaces sowie mit aktivem Seccomp-Allowlist über echte Prozesse ausführen (`crates/cave-kernel/tests/process_runtime.rs:1-101`), ergänzt um deduplizierende Allowlist-Unit-Tests (`crates/cave-kernel/src/lib.rs:1226-1283`). Die Bubblewrap-Integration erzwingt nun Namespaces, Cap-Drops und tmpfs/Bind-Mount-Policies mit konfigurierbaren Overrides (`crates/cave-kernel/src/lib.rs:1006-1183`, `crates/cave-kernel/src/lib.rs:1345-1408`, `crates/cave-daemon/src/main.rs:258-321`).
...
- Modell-Registry & Audit-Liste sind nun backend-seitig verdrahtet: `/api/v1/models` liefert Registrierung, Refresh, Löschung und Job-Verlauf, `/api/v1/audit/events` filtert signierte Einträge. Persistenz erfolgt über neue Datenbank-Helper & Migrationen für SQLite/Postgres (`crates/cave-daemon/src/server.rs:600-860`, `crates/bkg-db/src/lib.rs:240-880`, `crates/bkg-db/migrations/0009_model_registry.sql`, `crates/bkg-db/migrations/0010_audit_events.sql`).
...
- [x] Private-Key-Verschlüsselung für Blockchain-Konten auf AES-256-GCM mit PBKDF2 umgestellt.
  Status: `encrypt_private_key`/`decrypt_private_key` derivieren 256-bit-Schlüssel per PBKDF2 (100k Iterationen) und kapseln Salt+Nonce im Payload; Fehlerpfade & Tests validieren falsche Passphrasen (`crates/bkg-blockchain/src/crypto.rs:40-161`).
...
- [ ] Seccomp Profile und erweiterte Namespace-Isolation integrieren, um Bubblewrap-Fallback vollständig zu ersetzen.
  Status: Der Prozess-Runtime setzt OverlayFS + Seccomp-Allowlist um (`crates/cave-kernel/src/lib.rs:660-1183`) und Bubblewrap kapselt nun Namespaces, Cap-Drops sowie mountbare Pfade (`crates/cave-kernel/src/lib.rs:1006-1183`), inklusive ENV-Overrides im Daemon (`crates/cave-daemon/src/main.rs:258-321`, `crates/cave-daemon/src/server.rs:193-256`). Fertige Seccomp-Profile für Bubblewrap und zusätzliche Namespace-Validierung stehen aus.
