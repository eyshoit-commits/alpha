...
- [x] Audit-Log Format (signierte JSON-Lines) implementieren und überprüfen.
  Status: `AuditLogWriter` schreibt HMAC-signierte JSONL, rotiert mit Cosign-Signatur und lässt den Signaturlauf nun außerhalb des Mutex-Kontexts stattfinden (`crates/bkg-db/src/audit.rs:360-457`). Der Verifizierer kann einzelne Zeilen, komplette Dateien oder Streaming-Iterationen samt HMAC prüfen und bricht bei fehlenden Schlüsseln sauber ab (`crates/bkg-db/src/audit.rs:41-126, 590-767`).
