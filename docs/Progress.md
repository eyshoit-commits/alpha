...
- Der Daemon bedient jetzt das Model-Registry- und Audit-API vollständig via `bkg_db`: `/api/v1/models*` verwaltet Persistenz & Download-Jobs, `/api/v1/audit/events` liefert filterbare Audit-Logs, OpenAPI & Tests sind angepasst (`crates/cave-daemon/src/models.rs`, `crates/cave-daemon/src/audit.rs`, `crates/bkg-db/src/lib.rs`, `crates/bkg-db/migrations/0002_model_registry.sql`).
