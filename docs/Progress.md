...
- Overlay-Mounting räumt nun via RAII-Guard `OverlayMountGuard` in `ProcessSandboxInstance::exec` zuverlässig auf und ersetzt veraltete Helper-Verweise; Fehlerpfade unmounten damit automatisch (`crates/cave-kernel/src/lib.rs:832-955`). `cargo check -p cave-kernel` scheiterte aufgrund von `crates.io`-403-Antworten, der Offline-Check mangels lokalem `neo3-rs`-Cache (`chunk f89b70†L1-L10`, `chunk 1766ff†L1-L6`).
