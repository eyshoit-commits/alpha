...
- Phase-0 Komponenten sind teilweise implementiert: Das Prozess-Shim spannt jetzt Overlay-Workspaces und eine konfigurierbare Seccomp-BPF-Filterung auf (`crates/cave-kernel/src/lib.rs:604-1151`). Neu hinzugekommen sind Lifecycle-Tests, die den Prozess-Runtime ohne Namespaces sowie mit aktivem Seccomp-Allowlist über echte Prozesse ausführen (`crates/cave-kernel/tests/process_runtime.rs:1-101`), ergänzt um deduplizierende Allowlist-Unit-Tests (`crates/cave-kernel/src/lib.rs:1163-1179`). Namespace-Hardening via Bubblewrap bleibt offen.
